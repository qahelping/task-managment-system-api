# Allure отчетность для тестов

## 📑 Содержание

- [Установка Allure](#-установка-allure)
- [Запуск тестов с Allure](#-запуск-тестов-с-allure)
- [Структура Allure отчетов](#-структура-allure-отчетов)
- [Примеры использования](#-примеры-использования)
- [Просмотр отчетов](#-просмотр-отчетов)
- [Кастомизация](#-кастомизация)

## 📊 Установка Allure

### 1. Установите зависимости

```bash
pip install -r requirements.txt
```

### 2. Установите Allure CLI

**macOS:**
```bash
brew install allure
```

**Linux:**
```bash
# Скачайте с https://github.com/allure-framework/allure2/releases
# Или используйте SDKMAN:
sdk install allure
```

**Windows:**
```bash
# Скачайте с https://github.com/allure-framework/allure2/releases
# Распакуйте и добавьте в PATH
```

## 🚀 Запуск тестов с Allure

### Запуск всех тестов

```bash
pytest tests/ -v -s
```

Результаты сохраняются в `allure-results/`

### Генерация отчета

```bash
allure serve allure-results
```

Откроется браузер с интерактивным отчетом.

### Генерация статического отчета

```bash
allure generate allure-results -o allure-report --clean
allure open allure-report
```

## 📋 Структура Allure отчетов

### Features (Функциональности)
- **Аутентификация** - тесты входа и регистрации
- **Пользователи** - управление пользователями
- **Доски** - работа с досками
- **Задачи** - работа с задачами
- **Гостевой доступ** - права гостей
- **Управление участниками досок** - добавление/удаление участников
- **Архивация досок** - архивация и восстановление
- **Операции с задачами** - перенос, изменение статуса, приоритета
- **Поиск** - глобальный поиск
- **Статистика** - аналитика и статистика
- **Логи аудита** - логирование действий

### Stories (Истории)
Каждая feature разбита на stories - конкретные сценарии использования.

## 🎯 Примеры использования

### Запуск конкретной feature

```bash
pytest tests/ -v -s -k "test_auth"
```

### Запуск конкретной story

```bash
pytest tests/ -v -s -k "login"
```

### Запуск с маркерами

```bash
# Только smoke тесты
pytest tests/ -v -s -m smoke

# Только regression тесты
pytest tests/ -v -s -m regression
```

## 📝 Добавление маркеров в тесты

```python
import pytest

@pytest.mark.smoke
@allure.feature("Аутентификация")
def test_login():
    ...

@pytest.mark.regression
@allure.feature("Доски")
def test_create_board():
    ...
```

## 🔍 Просмотр отчетов

После запуска `allure serve allure-results` вы увидите:

1. **Overview** - общая статистика
2. **Suites** - группировка по файлам
3. **Graphs** - графики успешности
4. **Behaviors** - группировка по Features и Stories
5. **Packages** - группировка по пакетам

## 📊 Метрики в отчете

- Общее количество тестов
- Процент успешных/упавших тестов
- Время выполнения
- История запусков
- Скриншоты и логи (если добавлены)

## 🎨 Кастомизация

### Добавление описаний

```python
@allure.description("""
Этот тест проверяет создание доски обычным пользователем.
Доска должна быть создана успешно с правильными полями.
""")
def test_create_board():
    ...
```

### Добавление шагов

```python
@allure.step("Создание доски")
def create_board_step(title):
    response = requests.post(...)
    return response

def test_create_board():
    with allure.step("Получение токена"):
        token = get_user_token()
    
    with allure.step("Создание доски"):
        board = create_board_step("My Board")
    
    assert board.status_code == 201
```

### Прикрепление данных

```python
def test_example():
    response = requests.get(...)
    
    allure.attach(
        response.text,
        name="Response Body",
        attachment_type=allure.attachment_type.JSON
    )
```

## 🔗 Полезные ссылки

- [Документация Allure](https://docs.qameta.io/allure/)
- [Allure Python](https://github.com/allure-framework/allure-python)

```html 

html
 └── body
      ├── header
      ├── main
      │     └── div
      │         ├── p
      │         └── button
      └── footer

```